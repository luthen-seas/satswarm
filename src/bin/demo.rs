...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_tasks = vec![
        ("Protein Analysis", "Analyze the structure of protein BRCA1 for drug binding sites", 4000),
        ("Financial Modeling", "Optimize portfolio allocation with risk constraints for maximum yield", 3500),
        ("Machine Learning", "Train a neural network for image classification with 95% accuracy", 6000),
    ];
    ...
    for (domain, description, budget) in test_tasks {
        ...
        match protocol.process_task_request(description, budget, 120).await {
            Ok(task_id) => {
                let elapsed = start_time.elapsed();
                println!("✅ Task completed in {:.2}s - ID: {}", elapsed.as_secs_f64(), &task_id[..8]);
                
                // Show task details
                if let Some(task) = protocol.get_task_status(&task_id).await {
                    println!("   💰 Payment: {} sats", task.selected_bid.as_ref().unwrap().quoted_price_sats);
                    println!("   ⭐ Quality: {:.1}/5.0", task.verification_result.as_ref().unwrap().quality_score);
                    println!("   👤 Specialist: {}", &task.specialist_pubkey.as_ref().unwrap()[..12]);
                }
            }
            Err(e) => {
    ...
    let total_sats_transacted = active_tasks.iter()
    ...
    for i in 0..10 {
        ...
        match protocol_clone.process_task_request(&task_desc, 1000, 60).await {
            Ok(_) => {
                successful_tasks += 1;
                let task_time = start.elapsed();
                println!("   ✅ Task {} completed in {:.2}s", i + 1, task_time.as_secs_f64());
            }
            Err(e) => {
    ...
    println!("   ⏱️  Total time: {:.2}s", total_benchmark_time.as_secs_f64());
...
mod demo_utils {
    ...
    pub async fn run_concurrent_demo() -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("👥 Simulating {} concurrent users...", concurrent_tasks.len());
        ...
        for (user, task, budget) in concurrent_tasks {
            ...
            match protocol.process_task_request(task, budget, 90).await {
                Ok(task_id) => {
                    ...
                    println!("✅ {} task completed in {:.2}s", user, task_time.as_secs_f64());
    ...
    pub async fn run_stress_test() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for task_count in stress_levels {
            ...
            println!("✅ Completed {}/{} tasks in {:.2}s", successful, task_count, elapsed.as_secs_f64());
            println!("📈 Success rate: {:.1}%", (successful as f64 / task_count as f64) * 100.0);
            println!("🚀 Throughput: {:.1} tasks/sec", task_count as f64 / elapsed.as_secs_f64());
        }
