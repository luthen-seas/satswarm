...
impl TaskDecomposer {
        ...
        domain_rules.insert(
            ...
            DecompositionRule {
                ...
                subtask_templates: vec![
                    ...
                    SubTaskTemplate {
                        description: "Structure analysis and binding site identification"
                            .to_string(),
                        task_type: TaskType::Analysis,
                        skills: vec![
                            "structural_biology".to_string(),
                            "binding_analysis".to_string(),
                        ],
    ...
    fn calculate_complexity(&self, root: &TaskNode) -> f64 {
        ...
        let unique_skills: std::collections::HashSet<_> = root
            .subtasks
            .iter()
...
impl TaskTree {
    pub fn to_nostr_event(&self, keys: &Keys) -> Result<Event, Box<dyn std::error::Error>> {
        ...
        let tags = vec![
            Tag::generic(
                nostr::TagKind::Custom("task_type".into()),
                vec!["decomposition"],
            ),
            Tag::generic(
                nostr::TagKind::Custom("estimated_cost".into()),
                vec![&self.total_estimated_cost.to_string()],
            ),
            Tag::generic(
                nostr::TagKind::Custom("complexity".into()),
                vec![&format!("{:.2}", self.complexity_score)],
            ),
        ];

        let event = EventBuilder::new(Kind::Custom(30001), &content, tags).sign_with_keys(keys)?;
...
mod tests {
    ...
    fn test_protein_decomposition() {
        ...
        let result = decomposer
            .decompose("Analyze the structure of protein BRCA1")
            .unwrap();
    ...
    fn test_ml_decomposition() {
        let decomposer = TaskDecomposer::new();
        let result = decomposer
            .decompose("Train a machine learning model to predict stock prices")
            .unwrap();
